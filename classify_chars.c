...
void classify_chars(classifier_t* array, uint64_t size_of_array) {
    ...
    for (uint64_t i = 0; i < size_of_array; i++){
        classifier_t actual = array[i];
        //Tener en cuenta que el puntero vowels and consonants nos viene apuntando a NULL, 
        //por lo tanto, antes de configurar los arrays, habrá que pedir la memoria correspondiente.
        actual.vowels_and_consonants = malloc(2*sizeof(char*));
        classify_chars_in_string(actual.string, actual.vowels_and_consonants);
    }   
}
